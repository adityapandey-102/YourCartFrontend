...
router.post('/createUser',[
    ...
    if(user)
    {
        return res.status(400).json({ success:false,errors:[{msg:"Sorry user with this email is already exists"}]})
    }
...
router.post('/login',[
    ...
    try {
        ...
        if(!user)
        {
            return res.status(400).json({success:false,errors:[{msg:"Login with correct Credentials"}]});
        }
